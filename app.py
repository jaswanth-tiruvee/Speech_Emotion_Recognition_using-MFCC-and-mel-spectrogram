...
def main():
    ...
    if website_menu == "Emotion Recognition":
        ...
        if audio_file is not None:
            ...
            if model_type == "mel-specs":
                st.markdown("## Predictions")
                st.warning("We're still working on the model. It may not be working properly.")
    ...
    # elif website_menu == " Speech Emotion Classification":
    #     import pandas as pd
    ...
    #     txt = """
    #         * The problem of speech emotion can be solved by analyzing one or more of these
    #         features. Choosing to follow the lexical features would require a transcript of the
    #         speech which would further require an additional step of text extraction from speech 
    #         if one wants to predict emotions from real-time audio. Similarly, going forward with
    #         analyzing visual features would require the excess to the video of the conversations
    #         which might not be feasible in every case while the analysis on the acoustic features
    #         can be done in real-time while the conversation is taking place as we'd just need the
    #         audio data for accomplishing our task. Hence, we choose to analyze the acoustic
    #         features in this work.
    #         Furthermore, the representation of emotions can be done in two ways;
    #         * Discrete Classification: Classifying emotions in discrete labels like anger, happiness,
    #         boredom, etc.
    #         * Dimensional Representation: Representing emotions with dimensions such as
    #         Valence (on a negative to positive scale), Activation or Energy (on a low to high
    #         scale) and Dominance (on an active to passive scale)
    #     """

    #     st.subheader("Dataset")
    #     txt = """
    #         This web-application is a part of the IBM Hackathon April 2022**. 
